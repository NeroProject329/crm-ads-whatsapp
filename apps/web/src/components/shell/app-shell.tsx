...
type NavigationItem = Readonly<{
  label: string;
  href: string;
  icon: 'dashboard' | 'inbox' | 'ads' | 'leads' | 'numbers' | 'sites' | 'settings';
  adminOnly?: boolean;
  comingSoon?: boolean;
}>;
...
const navigation: readonly NavigationItem[] = [
  ...
  { label: 'Sites', href: '/sites', icon: 'sites', comingSoon: true },
  { label: 'Configuracoes', href: '/settings', icon: 'settings', adminOnly: true, comingSoon: true },
];
      ...
      <aside className={`app-sidebar${mobileOpen ? ' is-open' : ''}`}>
        ...
        <div className="workspace-chip">
          ...
          <div>
            ...
            <span>{loading ? 'Validando acesso...' : isAdmin ? 'Administrador' : 'Atendimento'}</span>
          </div>
        </div>
        ...
        <nav className="sidebar-nav" aria-label="Navegacao principal">
          ...
          {visibleNavigation.map((item) => {
            ...
            if (item.comingSoon) {
              return (
                <div className="nav-item is-disabled" key={item.href} aria-disabled="true">
        ...
        <div className="sidebar-footer">
          <div className="environment-status">
            ...
            <div>
              ...
              <small>Backend operacional</small>
            </div>
          </div>
      ...
      <div className="app-content-column">
        <header className="app-topbar">
          ...
          <div className="topbar-context">
            <span className="topbar-kicker">CRM ADS / WHATSAPP</span>
            <strong>Central de operacao</strong>
          </div>

          <div className="topbar-actions">
            <button className="icon-button" type="button" aria-label="Notificacoes" disabled>
              <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M6 17h12l-1.5-2.5V10a4.5 4.5 0 0 0-9 0v4.5L6 17Z" /><path d="M10 20h4" /></svg>
            </button>
            <div className="user-chip">
              <span className="user-avatar">{initials(user?.displayName)}</span>
              <div>
                <strong>{loading ? 'Carregando...' : (user?.displayName ?? 'Usuario')}</strong>
