...
export function TeamsManagement() {
  ...
  const employeeCountByTeam = useMemo(() => {
    const counts = new Map<string, number>();

    for (const employee of employees) {
      counts.set(employee.team.id, (counts.get(employee.team.id) ?? 0) + 1);
    }
  ...
  return (
    <div className="f2-page">
      <section className="f2-page-header">
        <div>
          <span className="f2-kicker">ESTRUTURA</span>
          <h1>Equipes</h1>
          <p>Organize os funcionarios em equipes e acompanhe a distribuicao da estrutura.</p>
        </div>
        <button className="f2-primary-button" type="button" onClick={createTeam}>
          + Nova equipe
        </button>
      </section>
      ...
      <section className="f2-team-grid">
        ...
        {!loading && teams.length === 0 ? (
          <div className="f2-panel f2-empty">Nenhuma equipe cadastrada.</div>
        ) : null}

        {teams.map((team) => (
          <article className="f2-team-card" key={team.id}>
            <div className="f2-team-top">
              <span className="f2-team-icon">{team.name.charAt(0).toUpperCase()}</span>
              <span className={`f2-status ${team.status.toLowerCase()}`}>
                {team.status === 'ACTIVE' ? 'ATIVA' : 'INATIVA'}
              </span>
            </div>
            <h2>{team.name}</h2>
            <p>{team.description || 'Sem descricao.'}</p>
            <div className="f2-team-meta">
              <div>
                <span>Funcionarios</span>
                <strong>{employeeCountByTeam.get(team.id) ?? 0}</strong>
              </div>
              <div>
                <span>Slug</span>
            ...
            <button className="f2-secondary-button" type="button" onClick={() => editTeam(team)}>
              Editar
            </button>
          </article>
        ))}
        ...
        <div className="f2-modal-backdrop">
          <section className="f2-modal">
            <div className="f2-modal-header">
              <div>
                <span className="f2-kicker">{editing ? 'EDITAR' : 'NOVA EQUIPE'}</span>
                <h2>{editing?.name ?? 'Criar equipe'}</h2>
              </div>
            ...
            <form className="f2-form" onSubmit={submit}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...
                        ...(!editing ? { slug: makeSlug(event.target.value) } : {}),
                      }))
                    }
                  />
                </label>
              ...
              <div className="f2-modal-actions">
                <button className="f2-secondary-button" type="button" onClick={() => setOpen(false)}>
                  Cancelar
                </button>
