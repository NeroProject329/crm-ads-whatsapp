...
async function verifySeed(): Promise<void> {
    ...
    select: {
      ...
      users: {
        ...
        select: {
          ...
          displayName: true,
          status: true,
          employee: { select: { employeeCode: true, status: true } },
  ...
  assert(adminUser?.status === 'INVITED', 'ADMIN must remain INVITED until Stage 2B.');
